...
*/
    ...
    function getTargetElements(target) {
        ...
        return dist_toArray(elements).reduce(((result, element) => [ ...result, ...dist_toArray(element).filter(dist_isNode) ]), []);
    }
    ...
    let split_type_dist_defaults = dist_extend(dist_defaults, {});
    class SplitType {
        static get data() {
            return cache;
        }
    ...
    function parallax() {
        ...
        items.forEach((item => {
            gsapWithCSS.to(item.querySelector("[data-parallax-image]"), {
                scale: 1.4,
                scrollTrigger: {
                    trigger: item,
                    start: "top 50%",
